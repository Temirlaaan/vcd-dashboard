...
def get_globally_used_ips_for_shared_pools() -> Dict[str, Set[str]]:
    """
    ИСПРАВЛЕНИЕ БАГА: Получить ВСЕ занятые IP для общих пулов со всех облаков.
    Это гарантирует, что если IP занят в одном облаке, он не будет показан как свободный в другом.
    """
    shared_pool_ips = {}
    
    # Определяем все общие пулы и их сети
    shared_networks = {}
    for cloud_config in CLOUDS_CONFIG.values():
        for pool in cloud_config["pools"]:
            if pool.get("shared_with"):
                network = pool["network"]
                if network not in shared_networks:
                    shared_networks[network] = {
                        "clouds": [pool.get("shared_with", [])],
                        "pools": []
                    }
                shared_networks[network]["pools"].append(pool["name"])
    
    logger.info(f"Found {len(shared_networks)} shared networks")
    
    # Инициализируем множества для каждой общей сети
    for network in shared_networks.keys():
        shared_pool_ips[network] = set()
    
    # Собираем ВСЕ занятые IP со ВСЕХ облаков для каждой общей сети
    for cloud_name, client in vcd_clients.items():
        ...
        try:
            for pool_config in config["pools"]:
                ...
                # Если это общий пул ИЛИ его сеть используется в другом облаке как общая
                if network in shared_networks:
                    allocations = client.get_pool_used_ips(pool_config)
                    for allocation in allocations:
                        shared_pool_ips[network].add(allocation.ip_address)
                    
                    logger.info(
                        f"Cloud {cloud_name}, pool {pool_config['name']}: "
                        f"added {len(allocations)} IPs to shared pool {network}"
                    )
        except Exception as e:
            logger.error(f"Error collecting shared pool IPs from {cloud_name}: {e}")
    
    # Логируем итоговую статистику
    for network, ips in shared_pool_ips.items():
        logger.info(f"Shared network {network}: total {len(ips)} used IPs across all clouds")
        ...
        for cloud_name, client in vcd_clients.items():
            ...
            try:
                ...
                for pool_config in pools:
                    ...
                    network = pool_config["network"]
                    if network in shared_pool_used_ips:
                        # Для общего пула используем ГЛОБАЛЬНЫЕ занятые IP
                        used_ips_set = shared_pool_used_ips[network]
                        logger.info(
                            f"Pool {pool_config['name']} in {cloud_name}: "
                            f"using {len(used_ips_set)} globally used IPs for shared network {network}"
                        )
                    else:
